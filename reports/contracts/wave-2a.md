...
2. `citation_rag/parse/pages.py`: split the document into pages at elements whose `style` matches either the legacy syntax `page-break-(before|after)\s*:\s*always` or the modern syntax `break-(before|after)\s*:\s*page` (case-insensitive), including `<hr>` with either style. Filings in the sample from the "CompSci Transform" generator use only the modern syntax. A filing with fewer than 15 markers is out of the corpus (wave 1b excludes them); the parser records `checks.stats.page_markers` anyway. Assign `index` 1..N. Find the printed `label`: check the last 3 and first 2 non-empty text lines of each page for a token matching `^(\d{1,3}|[A-Z]{1,2}-\d{1,3}|[ivxlc]{1,6})$` standing alone, or at the start or end of a short line (under 80 chars) that also contains the company name or "Form 10-K" or "|". Then enforce sequence: labels must increase by 1 across consecutive numbered pages (arabic and `F-` series separately); a label that breaks the sequence is set to null. Remove the label line, and remove lines that repeat on more than 30% of pages (running headers and footers), from the page text.
3. `citation_rag/parse/items.py`: detect Item headings. A heading is a block element (not inside a `<table>` that is the table of contents) whose text matches `^\s*(?:PART\s+[IV]+\s*[,.\-–—]?\s*)?ITEM\s+(\d{1,2}[A-C]?)\s*[.:\-–—]?\s*(.*)$` (case-insensitive) and is under 200 characters. Rules:
   ...
   - `filer_category` in the metadata is a `<br>`-joined string (for example "Non-accelerated filer<br>Smaller reporting company"); split it into a list before any comparison.
...
- Every sample filing that has >= 15 page markers (29 of 30) passes all prose checks, or the report names the reason for each failure. The one sample filing with no page markers is out of scope.
